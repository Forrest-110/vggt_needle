...
class Tensor(Value):
    ...
    @property
    def ndim(self):
        return len(self.realize_cached_data().shape)
    ...
    def unbind(self, dim: int = 0):
        return needle.ops.unbind(self, dim)

    def softmax(self):
        return needle.ops.softmax(self)
    
    def __getitem__(self, idx):
        return needle.ops.slice_tensor(self, idx)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for n in reverse_topo_order:
      cs = node_to_output_grads_list.get(n, [])
      if not cs:
          continue
      ...
      if not n.is_leaf():
          input_grads = n.op.gradient(tg, n)
          if not isinstance(input_grads, tuple):
            input_grads = (input_grads,)
          for inp, _g in zip(n.inputs, input_grads):
              if _g is None or not inp.requires_grad:
                  continue
              node_to_output_grads_list.setdefault(inp, []).append(_g)
