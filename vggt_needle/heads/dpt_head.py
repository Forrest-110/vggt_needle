...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        self.projects = nn.ModuleList(
            [nn.Conv(in_channels=dim_in, out_channels=oc, kernel_size=1, stride=1, padding=0) for oc in out_channels]
        )
        ...
        if feature_only:
            self.scratch.output_conv1 = nn.Conv(head_features_1, head_features_1, kernel_size=3, stride=1, padding=1)
        else:
            self.scratch.output_conv1 = nn.Conv(
    ...
    def scratch_forward(self, features: List[Tensor]) -> Tensor:
        """
...
class ResidualConvUnit(nn.Module):
    ...
    def __init__(self, features, activation, bn, groups=1):
        ...
        self.conv1 = nn.Conv(features, features, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv2 = nn.Conv(features, features, kernel_size=3, stride=1, padding=1, bias=True)
