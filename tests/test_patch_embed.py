...
#         needle_pe.proj.weight.data = Tensor(w_torch.astype("float32"))
...
if __name__ == "__main__":
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available for PyTorch, cannot run GPU test.")
    device_torch = torch.device("cuda")

    # Needle device
    device_ndl = ndl.cuda()
    print(f"Running tests on Torch device={device_torch}, Needle device={device_ndl}")
    ...
    class TorchPatchEmbed(nn_torch.Module):
        def __init__(
            ...
            self.norm = (
                nn_torch.LayerNorm(embed_dim) if use_norm else nn_torch.Identity()
            )
    ...
    def make_aligned_patch_embed(
        ...
        w_torch = torch_pe.proj.weight.detach().cpu().numpy()        # (O, I, H, W)
        # w_hwio = np.transpose(w_torch, (2, 3, 1, 0))                 # (H, W, I, O)
        needle_pe.proj.weight.data = Tensor(
            w_torch.astype("float32"),
            device=device_ndl,
        )

        if torch_pe.proj.bias is not None and needle_pe.proj.bias is not None:
            b_torch = torch_pe.proj.bias.detach().cpu().numpy()      # (O,)
            needle_pe.proj.bias.data = Tensor(
                b_torch.astype("float32"),
                device=device_ndl,
            )
        ...
        if use_norm:
            gamma_torch = torch_pe.norm.weight.detach().cpu().numpy()  # (D,)
            beta_torch = torch_pe.norm.bias.detach().cpu().numpy()     # (D,)
            needle_pe.norm.weight.data = Tensor(
                gamma_torch.astype("float32"),
                device=device_ndl,
            )
            needle_pe.norm.bias.data = Tensor(
                beta_torch.astype("float32"),
                device=device_ndl,
            )
    ...
    def run_forward_case(
        ...
        torch_pe, needle_pe = make_aligned_patch_embed(
            ...
            device_ndl=device_ndl,
        )

        # Random input on GPU
        np.random.seed(0)
        x_np = np.random.randn(B, C, H, W).astype("float32")
        x_torch = torch.tensor(x_np, device=device_torch)
        x_needle = Tensor(x_np, device=device_ndl)

        # Forward
        y_torch = torch_pe(x_torch).detach().cpu().numpy()
        y_needle = needle_pe(x_needle).numpy()  # assumed to return CPU numpy
    ...
    for case in cases:
        run_forward_case(*case, device_torch=device_torch, device_ndl=device_ndl)
